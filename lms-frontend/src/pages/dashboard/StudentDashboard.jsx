                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
                                {inProgress.map(e => (
                                    <div key={e.id} style={{ background: '#fff', padding: 32, borderRadius: '40px', border: '3px solid #F1F1F1', display: 'grid', gridTemplateColumns: '160px 1fr auto', gap: 32, alignItems: 'center', boxShadow: '0 8px 0 #F1F1F1' }} className="mobile-stack">
                                        ...
                                        <Link 
                                            to={e.progress >= 100 ? `/dashboard/course/${e.course_id || e.courseId}/final-exam` : `/dashboard/lesson/${e.last_watched_lesson_id || e.lastWatched || (e.course?.lessons?.[0]?.id || e.course?.lessons?.[0])}`} 
                            ...
                            <div style={{ background: '#001F3F', padding: 40, borderRadius: '48px', color: '#fff' }}>
                                ...
                                <div style={{ display: 'grid', gap: 16 }}>
                                    {certs.slice(0, 2).map(cert => (
                                        <div key={cert.id || cert._id} style={{ background: 'rgba(255,255,255,0.08)', padding: 20, borderRadius: '24px', display: 'flex', gap: 16, alignItems: 'center' }}>
